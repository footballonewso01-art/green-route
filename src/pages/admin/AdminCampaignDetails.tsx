    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={seedEdit} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border bg-surface px-3.5 text-sm font-semibold text-muted-foreground hover:text-foreground"><Edit3 className="h-4 w-4" /> Edit</button>
          {campaign.status !== "active" && campaign.status !== "ended" && <button type="button" onClick={() => void updateStatus("active")} disabled={saving} className="btn-primary-glow inline-flex h-10 items-center gap-2 px-4 py-0 text-sm"><Play className="h-4 w-4" /> Launch</button>}
          {campaign.status === "active" && <button type="button" onClick={() => void updateStatus("paused")} disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 text-sm font-semibold text-amber-300"><Pause className="h-4 w-4" /> Pause</button>}
          {campaign.status !== "ended" && <button type="button" onClick={() => void updateStatus("ended")} disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border bg-surface px-4 text-sm font-semibold text-muted-foreground hover:text-foreground"><Square className="h-3.5 w-3.5" /> End</button>}
        </div>
      </header>
