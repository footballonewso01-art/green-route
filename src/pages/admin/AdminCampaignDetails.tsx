...
export default function AdminCampaignDetails() {
  ...
  const [offerRewardEnabled, setOfferRewardEnabled] = useState(true);
  const [offerRewardPlan, setOfferRewardPlan] = useState("pro");
  const [offerRewardDays, setOfferRewardDays] = useState("14");
  const [offerMaxUses, setOfferMaxUses] = useState("0");
  ...
  const openOffer = () => {
    setOfferCode(""); setOfferRewardEnabled(true); setOfferRewardPlan("pro");
    setOfferRewardDays("14"); setOfferMaxUses("0"); setOfferOpen(true);
  };

  const saveOffer = async (event: FormEvent) => {
    event.preventDefault(); if (!campaign) return; setSaving(true);
    try {
      const updated = await pb.send<MarketingCampaign>(`/api/admin/campaigns/${campaign.id}/promocode`, {
        method: "POST",
        body: {
          code: offerCode,
          reward_enabled: offerRewardEnabled,
          reward_plan: offerRewardPlan,
          reward_days: offerRewardEnabled ? Number(offerRewardDays) : 0,
          max_uses: Number(offerMaxUses),
        },
        requestKey: null,
      });
      setCampaign(updated); setOfferOpen(false); toast.success("Campaign promocode created.");
    } catch (error) { toast.error(maskError(error, "Campaign promocode could not be created.")); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <Dialog open={offerOpen} onOpenChange={(open) => { if (!saving) setOfferOpen(open); }}><DialogContent className="max-w-xl border-border bg-surface p-0"><DialogHeader className="border-b border-border p-5 text-left"><DialogTitle>Create campaign promocode</DialogTitle><DialogDescription>The code must be unique across every partner, project campaign, and legacy offer.</DialogDescription></DialogHeader><form onSubmit={saveOffer}><div className="grid gap-4 p-5 sm:grid-cols-2"><label className="sm:col-span-2"><span className={labelClass}>Promocode</span><input required minLength={3} maxLength={32} value={offerCode} onChange={(e) => setOfferCode(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g, ""))} className={`${fieldClass} font-mono uppercase`} placeholder="AUTUMN14" autoFocus /></label><label className="sm:col-span-2 flex items-start gap-3 rounded-xl border border-border bg-background/35 p-3"><input type="checkbox" checked={offerRewardEnabled} onChange={(e) => setOfferRewardEnabled(e.target.checked)} className="mt-0.5 h-4 w-4 accent-[hsl(var(--accent))]" /><span><span className="block text-sm font-semibold text-foreground">Grant a signup reward</span><span className="mt-0.5 block text-xs text-muted-foreground">Turn this off for a tracking-only code.</span></span></label>{offerRewardEnabled && <><label><span className={labelClass}>Plan</span><select value={offerRewardPlan} onChange={(e) => setOfferRewardPlan(e.target.value)} className={fieldClass}><option value="pro">Pro</option><option value="agency">Agency</option></select></label><label><span className={labelClass}>Reward days</span><input required type="number" min={1} max={1095} value={offerRewardDays} onChange={(e) => setOfferRewardDays(e.target.value)} className={fieldClass} /></label></>}<label className="sm:col-span-2"><span className={labelClass}>Usage limit <span className="font-normal text-muted-foreground">· 0 is unlimited</span></span><input type="number" min={0} max={1000000} value={offerMaxUses} onChange={(e) => setOfferMaxUses(e.target.value)} className={fieldClass} /></label><p className="sm:col-span-2 text-xs leading-5 text-muted-foreground">This remains a project-owned offer with zero partner commission. Creating it does not modify partner attribution.</p></div><DialogFooter className="border-t border-border p-4"><button type="button" onClick={() => setOfferOpen(false)} className="h-10 rounded-xl border border-border px-4 text-sm font-semibold text-muted-foreground">Cancel</button><button type="submit" disabled={saving} className="btn-primary-glow inline-flex h-10 items-center gap-2 px-5 py-0 text-sm">{saving && <Loader2 className="h-4 w-4 animate-spin" />} Create promocode</button></DialogFooter></form></DialogContent></Dialog>
