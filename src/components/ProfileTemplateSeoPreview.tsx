...
function MiniPhone({ template }: { template: ProfileTemplateId }) {
  ...
  return (
    <div className={`relative mx-auto w-full max-w-[260px] overflow-hidden border border-white/10 bg-[#0a0d12] shadow-2xl ${cutout ? "rounded-[24px]" : "rounded-[32px]"}`}>
      ...
      {visual && <div className="absolute inset-0 bg-[linear-gradient(150deg,rgba(30,115,81,0.78),rgba(30,62,95,0.72)_48%,rgba(3,8,6,0.96)),radial-gradient(circle_at_75%_20%,rgba(255,255,255,0.22),transparent_28%)]" />}
      ...
      <div className={`relative px-5 pb-6 ${banner ? "-mt-9" : hero || template === "classic" ? "-mt-12" : visual ? "pt-10" : "pt-7"}`}>
        <div className={`${cutout ? "ml-auto h-32 w-24 rounded-t-[3rem] rounded-b-xl" : compact ? "h-16 w-16 rounded-full" : visual ? "h-[4.5rem] w-[4.5rem] rounded-full" : "h-20 w-20 rounded-full"} border-4 ${visual ? "border-white/25" : "border-[#0a0d12]"} bg-gradient-to-br from-emerald-300 to-cyan-600 shadow-xl ${banner ? "mx-auto" : cutout ? "" : "mx-auto"}`} />
        ...
        <div className="mt-5 space-y-2.5">
          {links.map((link, index) => (
            <div key={link} className={`${cutout ? "rounded-none border-x-0 border-t-0" : visual ? "rounded-2xl" : "rounded-xl"} flex items-center justify-between border border-white/10 bg-white/[0.09] px-3.5 py-3 text-xs font-bold text-white ${visual ? "backdrop-blur-xl" : ""}`}>
