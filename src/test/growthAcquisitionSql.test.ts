...
describe("growth acquisition SQL integration", () => {
  it("migrates, joins late signup enrichment to server activation, and rolls back", () => {
    ...
    const migrations = [
      "1787456000_add_growth_funnel.js",
      "1788600000_add_growth_landing_attribution.js",
      "1788620000_add_growth_campaign_content.js",
    ];
    ...
    record({ id: "signup:user1234567890", eventName: "signup_completed", landingPath: "/guides/how-to-track-link-clicks?secret=1", path: "/register?password=1", source: "google.com", medium: "organic", campaign: "cmp_launch", content: "plc_telegram" });
    record({ id: "signup:user1234567890", eventName: "signup_completed", landingPath: "/pricing", source: "bing.com", medium: "cpc", campaign: "cmp_other", content: "plc_other" });
    const result = db.prepare("SELECT * FROM growth_acquisition_funnel").get();
    expect(result).toMatchObject({ source: "google.com", medium: "organic", campaign: "cmp_launch", content: "plc_telegram", landing_path: "/guides/how-to-track-link-clicks", first_link_at: null });
    ...
    downs[2](app);
    expect(db.prepare("PRAGMA table_info(growth_events)").all().map((row) => row.name)).not.toContain("content");
    downs[1](app);
